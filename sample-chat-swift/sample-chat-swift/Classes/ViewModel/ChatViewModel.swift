...
class ChatViewModel: NSObject {
    ...
    private(set) var messages: DynamicArray<QBChatMessage> = DynamicArray(Array())
    ...
    func loadMoreMessages() {
        ...
        requestDownloadMessages = QBRequest.messagesWithDialogID(dialog.ID, extendedRequest: params, forPage: page, successBlock: { [weak self] (response: QBResponse!, downloadedMessages: [AnyObject]!, page: QBResponsePage!) in
            ...
            if let strongSelf = self, downloadedHistoryMessages =
                downloadedMessages as? [QBChatMessage]{
	...
	func loadRecentMessages() {
		...
		showLoadingIndicator.value = true
		var params = ["date_sent[gt]": self.messages.last!.dateSent.timeIntervalSince1970]
		...
		QBRequest.messagesWithDialogID(dialog.ID, extendedRequest: params, forPage: page, successBlock: {[weak self] (response: QBResponse!, downloadedMessages: [AnyObject]!, page: QBResponsePage!) -> Void in
			...
			if let strongSelf = self, downloadedRecentMessages = downloadedMessages as? [QBChatMessage] {
				strongSelf.showLoadingIndicator.value = false
    ...
    */
    ...
    var myLastMessage: QBChatMessage? { get {
